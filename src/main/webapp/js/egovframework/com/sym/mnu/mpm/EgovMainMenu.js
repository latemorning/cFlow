...
function createTree(arrName, vYeobu, checkValue) {
    ...
    if (treeNodes.length > 0) {
        ...
        if (startNode != 0) {
            ...
            vHtmlCode += "<div class='LeftMenuTitle'><font color='#00000'><b>"
                    + nodeValues[2] + "</b></font></div></td></tr>"
        } else {
            vHtmlCode += "<img src='" + imgpath
                    + "menu_base.gif' border='0' align='absbottom' alt='' >메뉴목록<br></td></tr>";
        }
    ...
    for (var i = 0; i < treeNodes.length; i++) {
        ...
        if (nodeValues[1] == parentNode) {
            ...
            for (g = 0; g < recursedNodes.length; g++) {
                vHtmlCodeEmpty += "<img src='" + imgpath
                        + "menu_empty.gif' border='0' align='absbottom' alt='' >";
            }
            ...
            if (hasChildNode) {
                ...
                if (recursedNodes.length == 1) {
                    vHtmlCode += vHtmlCodeBg
                            + "<a href=javascript:fn_MovePage('" + i + "');>"
                            + nodeValues[2] + "</a></td></tr>";
                } else {
                    vHtmlCode += vHtmlCodeBgList
                            + "<a href=javascript:fn_MovePage('" + i + "');>"
                            + nodeValues[2] + "</a></td></tr>";
                }
            }
...
function fn_MovePage(nodeNum) {
    ...
    let nodeValues = treeNodes[nodeNum].split("|");
    // parent.location.href = path + nodeValues[5];

    console.log(path + nodeValues[5]);

    window.parent.frames["main_right"].location.href = path + nodeValues[5];
    

    // window.parent.frames["main_top"].selectOne.vStartP.value = nodeValues[0];
    // document.selectOne.chkURL.value = url;
    // window.parent.frames["main_top"].selectOne.action = "/sym/mnu/mpm/EgovMainMenuRight.do";
    // window.parent.frames["main_top"].selectOne.target = "main_right";
    // window.parent.frames["main_top"].selectOne.submit();
