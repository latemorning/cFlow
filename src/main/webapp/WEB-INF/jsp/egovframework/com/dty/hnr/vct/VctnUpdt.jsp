...
t src="<c:url value='/js/egovframework/com/cmm/jqueryui.js' />"></scripㅁㅁㅁㅁㄴㄴㄴㅇㄹㄹㄹㄹt>
...
function fncEgovVcatnManageList(){
	location.href = "<c:url value='/dty/hnr/vct/VctnManageList.do'/>";
}
...
<form:form modelAttribute="vctnManage" name="vctnManage" method="post" action="${pageContext.request.contextPath}/uss/ion/vct/updtVcatnManage.do">
<input type="hidden" name="applcntIdKey" value="<c:out value='${vctnManageVO.applcntId}'/>" />
<input type="hidden" name="vcatnSeKey"   value="<c:out value='${vctnManageVO.vcatnSe}'/>" />
<input type="hidden" name="bgndeKey"     value="<c:out value='${vctnManageVO.bgnde}'/>" />
<input type="hidden" name="enddeKey"     value="<c:out value='${vctnManageVO.endde}'/>" />
...
<form:hidden  path="sanctnerId"/>
	...
	<!-- 타이틀 -->
	<h2 class=""><spring:message code="comUssIonVct.vcatnUpdt.title"/> VctnUpdt </h2><!-- 	휴가수정 -->
	...
	<table class="wTable mb20">
		...
		<tr>
			...
			<td class="left">
			    <c:out value='${vctnManageVO.applcntNm}'/>
			</td>
			<th><spring:message code="comUssIonVct.common.orgnztNm"/></th><!-- 소속 -->
			<td class="left">
			    <c:out value='${vctnManageVO.orgnztNm}'/>
			</td>
		</tr>
		<tr>
			<th><spring:message code="comUssIonVct.common.vcatnTotalInfo"/></th><!-- 신청자 연차정보 -->
			<td class="left" colspan="4">
				<span>[<spring:message code="comUssIonVct.common.occrncYrycCo"/>: ${vctnManageVO.occrncYrycCo}  ]</span><!-- 발생연차 -->
				<span style="margin-left:20px">[<spring:message code="comUssIonVct.common.useYrycCo"/>: ${vctnManageVO.useYrycCo   }  ]</span><!-- 사용연차 -->
				<span style="margin-left:20px">[<spring:message code="comUssIonVct.common.remndrYrycCo"/>: ${vctnManageVO.remndrYrycCo}  ]</span><!-- 잔여연차 -->
			</td>
		</tr>
		<tr>
			<th><spring:message code="comUssIonVct.common.vcatnSe"/> <span class="pilsu">*</span></th><!-- 휴가구분 -->
			<td class="left" colspan="3">
				<c:out value='${vctnManageVO.vcatnSeNm}'/>
				<input type="hidden" id="vcatnSe" name="vcatnSe" value="<c:out value='${vctnManageVO.vcatnSe}'/>" />
			</td>
		</tr>
		<tr>
			<th><spring:message code="comUssIonVct.common.startDate"/> <span class="pilsu">*</span></th><!-- 시작일자 -->
			<td class="left">
			    <c:out value='${vctnManageVO.bgnde}'/>
			</td>
			<c:if test="${vctnManageVO.vcatnSe ne '02' }">
				<th><spring:message code="comUssIonVct.common.endDate"/> <span class="pilsu">*</span></th><!-- 종료일자 -->
				<td class="left">
				    <c:out value='${vctnManageVO.endde}'/>
				</td>
			</c:if>
			<c:if test="${vctnManageVO.vcatnSe eq '02' }">
				<th><spring:message code="comUssIonVct.common.noonSe"/> <span class="pilsu">*</span></th><!-- 정오구분 -->
				<td class="left">
				    <c:if test="${vctnManageVO.noonSe eq '1' }"><spring:message code="comUssIonVct.common.noonSe1"/> </c:if><!-- 오전 -->
					<c:if test="${vctnManageVO.noonSe eq '2' }"><spring:message code="comUssIonVct.common.noonSe2"/> </c:if><!-- 오후 -->
				</td>
			</c:if>
		</tr>
		<tr>
			...
			<td class="left" colspan="3">
			    <textarea id="vcatnResn" name="vcatnResn" class="txaClass" rows="4" cols="70" title="<spring:message code="comUssIonVct.common.vcatnResn"/>"><c:out value='${vctnManageVO.vcatnResn}' escapeXml="false" /></textarea><!-- 휴가사유 -->
			</td>
		</tr>
	</table>
	...
	<c:import url="/uss/ion/ism/selectInfrmlSanctn.do" charEncoding="utf-8"> 
		<c:param name="infrmlSanctnId" value="${vctnManageVO.infrmlSanctnId}"/>
	</c:import>
